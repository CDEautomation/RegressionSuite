...
public class DocumentTypeFilter extends ActionDriver1{
	...
	@Test(priority = 1, description = "Verify Document Type Filter functionality")
	public void verifyDocumentTypeFilter() throws Throwable {
		try {
			dockethist = new DocketHistory(driver);
			boolean result = dockethist.verifyDocumentTypeFilter();
			Assert.assertEquals(result, true);
			getScreenshot(System.getProperty("user.dir") + "\\successScreenshots\\Verify_dockethistory_Document_Type_Filter.jpeg");
			Reporters.SuccessReport("DocketHistoryPage", "User Verified Docket History Document Type Filter Successfully");
		} catch (Exception e) {
			getScreenshot(System.getProperty("user.dir") + "\\FailedScreenshots\\Verify_product_type_filterfailed.ajpeg");
			Reporters.failureReport("Verify_Document_Type_Filter", "Unable to Verify_Document_Type_FilterScript");
		}
	}
