...
public class StatusFilter extends ActionDriver1{
	...
	public void verifyStatusFilter() throws Throwable {
		try {
			dockethist = new DocketHistory(driver);
			boolean result = dockethist.verifyStatusFilter();
			Assert.assertEquals(result, true);
			getScreenshot(System.getProperty("user.dir") + "\\successScreenshots\\Verify_dockethistory_Status_Filter.jpeg");
			Reporters.SuccessReport("DocketHistoryPage", "User Verified Docket History Status Filter Successfully");
		} catch (Exception e) {
			getScreenshot(System.getProperty("user.dir") + "\\FailedScreenshots\\Verify_status_filterfailed.ajpeg");
			Reporters.failureReport("Verify_Status_Filter", "Unable to Verify_Status_FilterScript");
		}
	}
