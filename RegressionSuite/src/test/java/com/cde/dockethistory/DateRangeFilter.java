...
public class DateRangeFilter extends ActionDriver1 {
	...
	@Test(priority = 1, description = "Verify Satatus Filter functionality")
	public void verifyStatusFilter() throws Throwable {
		try {
			dockethist = new DocketHistory(driver);
			boolean result = dockethist.verifyDateRangeFilter();
			Assert.assertEquals(result, true);
			getScreenshot(System.getProperty("user.dir") + "\\successScreenshots\\Verify_dockethistory_Date_Range_Filter.jpeg");
			Reporters.SuccessReport("DocketHistoryPage", "User Verified Docket History Date Range Filter Successfully");
		} catch (Exception e) {
			getScreenshot(System.getProperty("user.dir") + "\\FailedScreenshots\\Verify_date_range_filterfailed.ajpeg");
			Reporters.failureReport("Verify_Status_Filter", "Unable to Verify_Date_Range_FilterScript");
		}
	}
