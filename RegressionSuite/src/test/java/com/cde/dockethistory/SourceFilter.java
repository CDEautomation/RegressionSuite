...
public class SourceFilter extends ActionDriver1 {
	...
	@Test(priority = 1, description = "Verify Source Filter functionality")
	public void verifySourceFilter() throws Throwable {
		try {
			dockethist = new DocketHistory(driver);
			boolean result = dockethist.verifySourceFilter();
			Assert.assertEquals(result, true);
			getScreenshot(System.getProperty("user.dir") + "\\successScreenshots\\Verify_dockethistory_Source_Filter.jpeg");
			Reporters.SuccessReport("DocketHistoryPage", "User Verified Docket History Source Filter Successfully");
		} catch (Exception e) {
			getScreenshot(System.getProperty("user.dir") + "\\FailedScreenshots\\Verify_source_filterfailed.ajpeg");
			Reporters.failureReport("Verify_Source_Filter", "Unable to Verify_Source_FilterScript");
		}
	}
