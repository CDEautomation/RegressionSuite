...
public class DistrictFilter extends ActionDriver1 {
	...
	@Test(priority = 1, description = "Verify District Filter functionality")
	public void verifyDistrictFilter() throws Throwable {
		try {
			dockethist = new DocketHistory(driver);
			boolean result = dockethist.verifyFilterDist();
			Assert.assertEquals(result, true);
			getScreenshot(System.getProperty("user.dir") + "\\successScreenshots\\Verify_dockethistory_dist_Filter.jpeg");
			Reporters.SuccessReport("DocketHistoryPage", "User Verified Docket History District Filter Successfully");

		} catch (Exception e) {
			getScreenshot(System.getProperty("user.dir") + "\\FailedScreenshots\\Verify_district_filterfailed.jpeg");
			Reporters.failureReport("Verify_District_Filter", "Unable to Verify_District_FilterScript");
		}
	}
